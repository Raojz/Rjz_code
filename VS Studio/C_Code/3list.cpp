...
//双向链表
typedef struct doubele_list
{
    int data;
    struct doubele_list* prev;
    struct doubele_list* next;
}D_List;

D_List* D_List_insert_head(D_List*node,int num)//头插法
{
    D_List* new_node = (D_List*)malloc(sizeof(D_List));
    if (new_node == NULL) {
        printf("内存分配失败！\n");
        return node;
    }
...
D_List* D_List_insert_tail(D_List* node, int num)//尾插法
{
    D_List* new_node = (D_List*)malloc(sizeof(D_List));
    new_node->data = num;
    new_node->next = NULL;
    if (node == NULL)//新节点就是首节点
    {
        new_node->prev = NULL;
        return new_node;
    }
    D_List* current = node;
    while (current->next != NULL)//找最后一个节点
    {
        current = current->next;
    }
    current->next = new_node;
...
D_List* D_List_del_node(D_List* node, int num)//删除特定数据节点
{
    if (node == NULL)//空链表
    {
        return NULL;
    ...
    // 情况1：删除头节点
    if (temp->prev == NULL) {      
        D_List* new_head = temp->next;// 新的头节点是原头节点的下一个
        if (new_head != NULL) {
            new_head->prev = NULL;
        }
        free(temp);
    ...
    temp->prev->next = temp->next;//将要删除节点的前节点和后节点链接
    
    if (temp->next != NULL) // 如果不是尾节点，更新后继节点的前驱
    {
...
void traverse_Dlist(D_List* head)
{
    D_List* current = head;
    printf("正向遍历：");
    while (current != NULL) {
...
// 反向遍历
void traverseBackward(D_List* tail) //先找到尾节点
{
    if (tail == NULL) {
        return;
    }
...
void D_list_code1()
{
    ...
    while (1)
    {
        ...
        case 1:
            printf("请输入要插入的数据：");
            scanf("%d", &data);
            head = D_List_insert_head(head, data);
            printf("插入成功！\n");
            break;
        ...
        case 2:
            printf("请输入要插入的数据：");
            scanf("%d", &data);
            head = D_List_insert_tail(head, data);
            printf("插入成功！\n");
            break;
        ...
        case 3:
            printf("请输入要删除的数据：");
            scanf("%d", &data);
            head = D_List_del_node(head, data);
            printf("删除成功！\n");
            break;
